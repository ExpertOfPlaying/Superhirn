...
class TestValidatorClass(unittest.TestCase):
    def setUp(self):
        ...
        role = Role
        self.validator_rater = Validator(rule_book, validation_error, game_state, role.Rater)
        self.validator_coder = Validator(rule_book, validation_error, game_state, role.Coder)
    ...
    def test_check_game_state_correct_guess_rater(self):
        self.assertTrue(self.validator_rater.check_game_state(self.board.attempt_counter, self.board.guessed_code,
                                                              self.board.code).value)

    def test_check_game_state_incorrect_guess_rater(self):
        self.assertFalse(self.validator_rater.check_game_state(self.board.attempt_counter, self.bad_stone_input_string,
                                                               self.board.code))
    ...
    def test_check_game_state_coder(self):
        self.board.attempt_counter = 11
        self.assertTrue(self.validator_coder.check_game_state(self.board.attempt_counter, self.bad_stone_input_string,
                                                              self.board.code).value)

    def test_check_game_state_incorrect_guess_coder(self):
        self.assertFalse(self.validator_coder.check_game_state(self.board.attempt_counter, self.bad_stone_input_string,
                                                               self.board.code))

    def test_check_game_state_lose_coder(self):
        self.assertFalse(self.validator_coder.check_game_state(self.board.attempt_counter, self.board.guessed_code,
                                                               self.board.code).value)

    # check_stone_input
    def test_check_stone_input_good(self):
        self.assertTrue(self.validator_rater.check_stone_input(self.good_stone_input_string, 1, self.board.max_colour))

    def test_check_stone_input_bad_out_of_bounds(self):
        with self.assertRaises(ValidationError) as exception:
            self.validator_rater.check_stone_input(self.bad_out_of_bounds_stone_input_string, 1, self.board.max_colour)
        exception_message = exception.exception
        print(exception_message)

    def test_check_stone_input_bad_char(self):
        with self.assertRaises(ValidationError) as exception:
            self.validator_rater.check_stone_input(self.bad_char_stone_input_string, 1, self.board.max_colour)
        exception_message = exception.exception
        print(exception_message)

    # check_code_input
    def test_check_code_input_good(self):
        self.assertTrue(self.validator_rater.check_code_input(
    ...
    def test_check_code_input_bad_length_high(self):
        with self.assertRaises(ValidationError) as exception:
            self.validator_rater.check_code_input(self.bad_high_stone_input_string, self.board.code_max_length,
    ...
    def test_check_code_input_bad_length_low(self):
        with self.assertRaises(ValidationError) as exception:
            self.validator_rater.check_code_input(self.bad_low_stone_input_string, self.board.code_max_length,
                                                  self.board.max_colour)
        exception_message = exception.exception
        print(exception_message)
    ...
    def test_check_code_input_bad_out_of_bounds(self):
        with self.assertRaises(ValidationError) as exception:
            self.validator_rater.check_code_input(
    ...
    def test_check_feedback_input_good(self):
        self.assertTrue(
            self.validator_coder.check_feedback_input(self.good_feedback_input_string, self.board.code_max_length))

    def test_check_feedback_input_bad_high(self):
        with self.assertRaises(ValidationError) as exception:
            self.validator_coder.check_feedback_input(
